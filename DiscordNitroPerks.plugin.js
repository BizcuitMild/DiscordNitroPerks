/**
 * @name DiscordNitroPerks
 * @website https://github.com/BizcuitMild/DiscordNitroPerks
 * @source https://raw.githubusercontent.com/BizcuitMild/DiscordNitroPerks/main/DiscordNitroPerks.plugin
 * @updateUrl https://raw.githubusercontent.com/BizcuitMild/DiscordNitroPerks/main/DiscordNitroPerks.plugin
 */
/*@cc_on
@if (@_jscript)
...
module.exports = (() => {
    const config = {
        "info": {
            ...
            "authors": [{
                "name": "ซามายด์.",
                "discord_id": "633177241634865153",
                "github_username": "BizcuitMild"
            }],
            "version": "1.0.0",
            "description": "ปลดล็อกโหมดแชร์หน้าจอทั้งหมด, และใช้ emotes ข้ามเซิร์ฟเวอร์และ gif emotes, ปล. คุณไม่สามารถอัปโหลดไฟล์ 100MB ได้ ;w;",
            "github": "https://github.com/BizcuitMild/DiscordNitroPerks",
            "github_raw": "https://raw.githubusercontent.com/BizcuitMild/DiscordNitroPerks/main/DiscordNitroPerks.plugin"
        },
        "main": "DiscordNitroPerks.plugin.js"
    };
    ...
    return !global.ZeresPluginLibrary ? class {
        ...
        load() {
            BdApi.showConfirmationModal("Library Missing", `The library plugin needed for ${config.info.name} is missing. Please click Download Now to install it.`, {
                confirmText: "ดาวน์โหลดเดี๋ยวนี้",
                cancelText: "ยกเลิก",
                onConfirm: () => {
                ...
                getSettingsPanel() {
                    return Settings.SettingPanel.build(_ => this.saveAndUpdate(), ...[
                        new Settings.SettingGroup("Features").append(...[
                            new Settings.Switch("การแบ่งปันหน้าจอคุณภาพสูง", "เปิดหรือปิด การแชร์หน้าจอ 1080p / source @ 60fps สิ่งนี้ปรับให้เข้ากับสถานะไนโตรปัจจุบันของคุณ", this.settings.screenSharing, value => this.settings.screenSharing = value)
                        ]),
                        new Settings.SettingGroup("Emojis").append(
                            new Settings.Switch("Nitro Emotes Bypass", "เปิดหรือปิด ใช้งานโดยใช้ Nitro Emote bypass", this.settings.emojiBypass, value => this.settings.emojiBypass = value),
                            new Settings.Slider("ขนาด", "ขนาดของอีโมจิเป็นพิกเซล แนะนำให้ใช้ 40", 16, 64, this.settings.emojiSize, size=>this.settings.emojiSize = size, {markers:[16,20,32,40,64], stickToMarkers:true})
                        ),
                            new Settings.SettingGroup("Profile Picture").append(...[
                                new Settings.Switch("รูปภาพโปรไฟล์ฝั่งไคลเอ็นต์", "เปิดหรือปิด รูปภาพโปรไฟล์ฝั่งไคลเอ็นต์", this.settings.clientsidePfp, value => this.settings.clientsidePfp = value),
                                new Settings.Textbox("URL", "URL โดยตรงที่มีรูปโปรไฟล์ที่คุณต้องการ", this.settings.pfpUrl,
                                    image => {
                                        try {
                                            new URL(image)
                                        } catch {
                                            return Toasts.error('URL ที่ไม่ถูกต้อง!')
                                        }
